 ...
 * @author basic
 */
public class ViewUserMessagesForm extends javax.swing.JFrame {
  private ArrayList<Integer> user_ids = new ArrayList<>();
  /**
   * Creates new form ViewUserMessagesForm
   */
  public ViewUserMessagesForm() {
    initComponents();
    // get all users that sent messages
    try {
      user_ids = UserController.getInstance().getAllUserIDsThatSentMessages();
      if(user_ids.isEmpty()) {
        ...
        ArrayList<String> subjects = UserController.getInstance().getAllSubjectsByUserID(first_user.getID());
        for(String subject : subjects) {
          subject_combo_box.addItem(subject);
        }
        // get first subject and fill message field
        message_field.setText(UserController.getInstance().getMessageBySubject(subjects.get(0)));
      }
    } catch (Exception e) {
  ...
  private void users_combo_boxItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_users_combo_boxItemStateChanged
    ...
    try {
      ...
      ArrayList<String> subjects = UserController.getInstance().getAllSubjectsByUserID(user.getID());
      for(String subject : subjects) {
        subject_combo_box.addItem(subject);
      }
      // get first subject and fill message field
  ...
  private void subject_combo_boxItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_subject_combo_boxItemStateChanged
    // TODO add your handling code here:
    try {
      String message = UserController.getInstance().getMessageBySubject(subject_combo_box.getSelectedItem().toString());
  ...
  public static void main(String args[]) {
    ...
    try {
      ...
      java.util.logging.Logger.getLogger(ViewUserMessagesForm.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
    ...
    java.awt.EventQueue.invokeLater(new Runnable() {
      public void run() {
        new ViewUserMessagesForm().setVisible(true);
      }
    });
  }
